...
# =============================================================
# PWA Mobile Patch — 手机 PWA 前端补丁
# =============================================================
# 在原作者最新版基础上加的最小补丁。
# - 挂载 /app/ 路径到 static/ 文件夹下的 PWA 前端
# - 复用原作者的 cookie session 认证（不引入第二套认证）
# - 补充几个原版缺失的 HTTP 路由：PATCH/DELETE bucket、POST hold（用于手机端新建）
# =============================================================
...
async def _pwa_app_redirect(request):
    """无尾斜杠时重定向到 /app/"""
    from starlette.responses import RedirectResponse
...
@mcp.custom_route("/app/", methods=["GET"])
async def _pwa_serve_index(request):
    """PWA 入口"""
    from starlette.responses import FileResponse, JSONResponse
    index = os.path.join(_STATIC_DIR, "index.html")
    if not os.path.exists(index):
        return JSONResponse({"error": "PWA frontend not deployed"}, status_code=404)
    return FileResponse(index)


@mcp.custom_route("/app/{filename}", methods=["GET"])
async def _pwa_serve_static(request):
    """PWA 静态资源"""
    from starlette.responses import FileResponse, JSONResponse
...
async def _pwa_update_bucket(request):
    """修改记忆桶元数据或内容（手机端编辑用）"""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err:
    ...
    if not updates:
        return JSONResponse({"error": "没有需要修改的字段"}, status_code=400)

    success = await bucket_mgr.update(bucket_id, **updates)
    if not success:
        return JSONResponse({"error": "修改失败"}, status_code=500)

    return JSONResponse({"id": bucket_id, "updated": list(updates.keys())})


@mcp.custom_route("/api/bucket/{bucket_id}", methods=["DELETE"])
async def _pwa_delete_bucket(request):
    """删除记忆桶（手机端删除用）"""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err:
...
async def _pwa_create_bucket(request):
    """新建记忆桶（手机端新建用，等同于 MCP 的 hold 工具）"""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err:
        return err
    ...
    elif not isinstance(extra_tags, list):
        extra_tags = []
    ...
    try:
        ...
        logger.warning(f"PWA: Auto-tagging failed: {e}")
        analysis = {
            "domain": ["未分类"], "valence": 0.5, "arousal": 0.3,
            "tags": [], "suggested_name": "",
        }
    ...
    if pinned:
        ...
        return JSONResponse({
            "id": bucket_id,
            "action": "pinned",
            "name": suggested_name or bucket_id,
            "domain": domain,
        })
